...
- **Task 4** is `[~]` for the identical reason: the parser is fully implemented and tested against every Task 3 fixture (compliant + all 40 violating variants), but the "and all real documents" half of its "Done when" clause can't be exercised until those same real manuscripts exist. No known gap in the code — this is a test-coverage extension waiting on external input, not a design shortfall.
...
- **Done when:** every §5.1 field populated on the compliant fixture ✅ and all real documents ⏳ (blocked on the user-supplied files from Task 3) · resolution tests cover all four inheritance sources including theme fonts ✅ · parse→parse ID stability ✅ · input SHA-256 identical before and after parse (FR-9, asserted not assumed) ✅
...
- `[x]` Front-matter state machine for title/author/abstract/affiliation — these have no headings (or, for abstract, one too short and unstyled to score reliably); score signals (superscript author-line numerals, ORCID/email for affiliation, an explicit "Abstract"/"Summary" heading), don't use fixed indices. **Deviation from this bullet's original wording:** corresponding-author is *not* handled here — in the compliant fixture it carries a real, bold, uppercase "CORRESPONDING AUTHOR" heading that already matches `corresponding_author_address`'s configured synonyms, so it resolves through the ordinary per-paragraph heading scan in `section_index.segment`, with no special-casing needed
- `[x]` **Synonym tables and `section_order` live in the rule config JSON, not in code** — `journal_v1.json` gained an `"abstract": ["abstract", "summary"]` entry so the Abstract heading resolves through the same vocabulary path as every body heading, rather than a hardcoded string in the segmenter
...
- **Done when:** all 14 labels assignable ✅ (compliant fixture) · tested against compliant, scrambled-order (`violating("doc-order")`), and heading-variant (synonym-substituted heading text) fixtures ✅ and all real documents ⏳ (blocked on the user-supplied files from Task 3, same as Tasks 3/4)
- **Verified 2026-09-12:** 269 tests pass (19 new) · ruff and mypy --strict clean
